...
const APIKeyTable = ({ apiKeys }: { apiKeys: APIKey[] }) => {
	...
	const getVisiblePermissions = (apiKey: APIKey) =>
	...
	return (
		<motion.div variants={itemVariants} initial="hidden" animate="visible">
			<Table className="mt-6">
				...
				<TableBody>
					{apiKeys.map((apiKey) => {
						const visiblePermissions = getVisiblePermissions(apiKey);
							...
							<TableRow key={apiKey.id}>
								...
								<TableCell>
									<div className="flex flex-wrap items-center gap-2 max-w-[400px]">
										{visiblePermissions.slice(0, 2).map((permission) => (
											<span
												key={permission}
										...
										{visiblePermissions.length > 2 && (
											<button
												...
												aria-label={`${visiblePermissions.length - 2} more permissions`}
											>
												+{visiblePermissions.length - 2}
											</button>
										)}
						...
						>
							<DialogContent className="sm:max-w-[480px]">
								...
								<div className="flex flex-wrap gap-2">
									{getVisiblePermissions(selectedApiKey).map((permission) => (
										<span
											key={permission}
											className="inline-flex items-center bg-primary text-secondary text-xs leading-none px-2 pt-1 pb-1.5 rounded-md whitespace-nowrap"
										>
