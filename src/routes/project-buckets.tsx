...
} from "@/features/project/services/project-files.dto";
...
export default function ProjectBucketsPage() {
	...
	const runRagQuery = async () => {
		...
		if (!normalizedQuery) {
			toast.error("Enter a query to search the RAG.");
			return;
		}
		...
		try {
			...
			toast.error(
				error instanceof Error ? error.message : "Failed to query the RAG."
			);
		}
	};
		...
		<DashboardLayout pageTitle={t("sidebar:project.buckets.title", "Buckets")}>
				...
				<Tabs defaultValue="storage" className="w-full">
					...
					<TabsContent value="rag">
						<Card>
							<CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
								<div>
									<CardTitle>RAG Data</CardTitle>
									<CardDescription>
										Files successfully uploaded and extracted into the Vector
										search engine.
									</CardDescription>
								</div>
								<Button onClick={() => setQueryDialogOpen(true)}>Query</Button>
							</CardHeader>
							<CardContent>
								<div className="rounded-lg border border-border">
									<Table>
			...
			<Dialog open={queryDialogOpen} onOpenChange={setQueryDialogOpen}>
				<DialogContent className="sm:max-w-[720px]">
					<DialogHeader>
						<DialogTitle>Query RAG</DialogTitle>
						<DialogDescription>
							Search the RAG and view the matching chunks returned by the
							backend.
						</DialogDescription>
					</DialogHeader>
					...
					>
						<div className="grid gap-2">
							...
							<div className="flex gap-2">
								<Input
									id="rag-query-input"
									placeholder="Ask something about the RAG content"
									value={queryText}
